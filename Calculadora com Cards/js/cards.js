...
function equacao2grau(event) {
    ...
    resultado6 = alert("A soma dos números resulta em: " + delta);
    x1 = (-num9 + Math.Sqrt(delta)) / 2 * num8;
    x2 = (-num9 - Math.Sqrt(delta)) / 2 * num8;
    
    if (delta > 0) {
        resultado6 = alert("Duas Raízes reais foram obtidas:", x1, x2);;
    }
    else if (delta == 0) {
        resultado6 = alert("Duas Raízes reais iguais foram obtidas:", x1, x2);
    }
    else{
        resultado6 = alert("Não possui raízes reais");
    }
